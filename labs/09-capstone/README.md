# Module 09 — Capstone: Agent Mode End-to-End 🔴

**Time:** 2–3 hours · **Difficulty:** Advanced · **Surfaces:** Agent mode + everything from M01–M08
...
> - **Frontend:** A new tab in `sample-app-frontend/` that fetches the preview and renders each SKU with regular and best price (uses the M04 `PriceBadge` extensions).
...
- [ ] `AGENTS.md` from M03 includes "must run `mvn test` / `dotnet test` / `npm test` before declaring done"
- [ ] **Reviewer** and **Security Auditor** modes available
- [ ] GitHub MCP server connected (M08)
- [ ] Frontend rule from M03 in place (typed client + a11y)
...
4. Trigger your M08 PR-review workflow. Confirm the bot comment appears.
...
- Add an "approve-if-trivial" path in the M08 PR-review Action.
